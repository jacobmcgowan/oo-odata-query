...
test('Expand one child', () => {
  expect(new Expand([new Expansion('address')]).toString()).toBe('address');
});
...
test('Expand multiple children', () => {
  expect(new Expand([new Expansion('contacts'), new Expansion('address')]).toString()).toBe('contacts,address');
});
